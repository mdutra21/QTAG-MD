...
!pn(:,:)
complex*16, allocatable :: c(:), co(:), ov_no(:,:), edim(:)
logical :: ireex
...
call energy_calc(nbt, qpas(1:nbt,:,:), c(1:nbt), einit)
call normalize(nbt, qpas(1:nbt,:,:), c(1:nbt), norm)
!call reex_traj(nbt, gbc(1:nbt,1:ndof), c(1:nbt), ntrk)
!call energy_comp(nbt, gbc(1:nbt,1:ndof), c(1:nbt), ke0, v0)
...
write(401,form1) 0d0, einit
write(406,form1) t, zcorr
...
!idedq = idedq+1
call basis_diag(nbt, qpas(1:nbt,:,:), co(1:nbt), c(1:nbt), evals(1:nbt))
call energy_calc(nbt, qpas(1:nbt,:,:), c(1:nbt), ediag)
...
!For KE reexpansions:
!call energy_comp(nbt, qpas(1:nbt,:,:), c(1:nbt), ket, vt)
...
call autocorr(nbt, qpas(1:nbt,:,:), c(1:nbt), zcorr, tcorr)
call avg_pos(nbt, qpas(1:nbt,:,:), c(1:nbt), xavg)
!call psi_update(nbt,qpas(1:nbt,:,:),c(1:nbt),t,205)
!end if

do i=1,nbt
  write(205,form1) t, (qpas(i,n,1), n=1,ndof)
